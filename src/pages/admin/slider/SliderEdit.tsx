...
const SliderEdit = () => {
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    axios.patch(`admin/sliders/${_id}`,formData).then((response) => navigate("/admin/sliders"))
  ...
  const getCurrentData = () => {
    axios.get(`admin/sliders/${_id}`)
    .then(({data}) => {
        setSlider(data.data)
        setCheckbox(data.data.status)
        setPreviewImage(imagePath(data.data.image))
    })
    .catch(error => console.log(error))
  }
  useEffect(() => {
  ...
  return (
    <div className='w-[90%] rounded-md shadow-sm my-5 p-10 mx-auto border-gray-100 border'>
                  <h1 className='text-3xl w-full mb-6 text-center  text-emerald-400'>Slider Edit Page</h1>
      {previewImage ? (
      ...
      <form onSubmit={handleSubmit} className='justify-between flex flex-wrap'>
                <div className='w-[100%] my-5'>
                    ...
                    <input className='file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-violet-50 file:text-emerald-400 hover:file:bg-violet-100  w-full bg-transparent outline-none px-2 py-1.5 border-b border-gray-400' type="file" name="image" onChange={handleFileChange} accept='image/*' />
                </div>
                <div className='w-[40%] my-5'>
                    <label htmlFor="title" className='block'>Title</label>
                    <input className='w-full bg-transparent outline-none px-2 py-1.5 border-b border-gray-400' name="title" defaultValue={slider.title} required type="text" onChange={handleInputChange} />
                </div>
      ...
      />
                ...
                <div className='w-[40%] my-5'>
                    ...
                    <input className='w-full  bg-transparent outline-none px-2 py-1.5 border-b border-gray-400' type="number" defaultValue={slider.order_by} required min={1} name="order_by" onChange={handleInputChange}/>
                </div>
                <div className='w-[100%] my-5'>
                    <label htmlFor="description" className='block'>Description</label>
                    <textarea rows={6} defaultValue={slider.description} required name="description" className='w-full bg-transparent outline-none px-2 py-1.5 border-b border-gray-400' onChange={handleInputChange}/>
                </div>
                <button className='border w-full mx-auto text-center text-emerald-400 font-medium border-emerald-400 rounded-md py-1'><AiOutlinePlus className='mr-2 mb-1 inline' />Edit Slider</button>
            </form>
    </div>
  )
}
